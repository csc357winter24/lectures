/* To copy the contents of one file into another using system calls instead
...
int main(int argc, char *argv[]) {
    ...
    /* The system calls return only a file descriptor (an index into the open
     *  file table), not a pointer to a FILE structure: */
    src = open(argv[1], O_RDONLY);
    dest = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);

    /* The system calls expect a file descriptor, not a pointer to a FILE: */
